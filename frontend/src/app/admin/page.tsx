    ...
    const handleImageUpload = async (e: any) => {
      ...
      try {
        const res = await Promise.all(
          file.map((file) => uploadImage(file).then((r) => r.url))
        );
          ...
          <div className="flex gap-2 flex-wrap mt-2 mb-2">
            {images.map((url, i) => (
              <div key={i} className="relative">
                <img
                  src={url}
                  className="w-20 h-20 object-cover rounded-lg border-2 border-[#d6cfc4]"
                />

                {i === 0 && (
                  <span className="absolute bottom-0 left-0 right-0 text-center text-white text-[10px] bg-[#b08968] rounded-b-lg">
                    Principal
                  </span>
                )}
          ...
          {products.map((product: any) => (
            ...
            >
              ...
              <div className="p-3 flex flex-col flex-grow">
                <h3 className="text-sm md:text-base text-[#5a4634] font-semibold line-clamp-2 min-h-[2.5rem] leading-tight">
                  {product.name}
                </h3>
