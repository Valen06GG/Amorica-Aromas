          ...
          <div className="flex gap-3 p-4">
            {allImages.length > 1 && (
              <div className="flex flex-col gap-2">
                {allImages.map((url, i) => (
                  <img key={i} src={url} onClick={() => setSelected(url)}
            ...
            <div className="flex-1 overflow-hidden rounded-xl">
              <img src={selected} alt={product.name} className="w-full h-[400px] object-cover hover:scale-105 transition duration-500" />
            </div>
          </div>
